...
public class Request {
    ...
    private final RawHttpHeaders headers;                      // Request Headers
    ...
    public RawHttpExchange exchange;
    ...
    public Request(RawHttpExchange exchange) {
        ...
        this.secure = this.protocol == "HTTPS"; // Can be suckered?
        this.exchange = exchange;
        // Parse content length
        String contentLength = headers.get("Content-Length") != null && !headers.get("Content-Length").isEmpty() ? headers.get("Content-Length").get(0) : null;
        this.contentLength = contentLength != null ? Long.parseLong(contentLength) : -1;

        // Check if the request contains an body-content
        this.contentType = headers.get("Content-Type") == null  || headers.get("Content-Length").isEmpty() ? "" : headers.get("Content-Type").get(0);
    ...
    public boolean isFresh() {
        ...
        if (headers.contains("cache-control") && headers.get("cache-control").get(0) != null && headers.get("cache-control").get(0).equals("no-cache"))
            return true;

        if (headers.contains("if-none-match") && headers.get("if-none-match").get(0) != null && headers.get("if-none-match").get(0).equals("*"))
            return true;

        if (headers.contains("if-modified-since") && headers.contains("last-modified") && headers.contains("modified")) {
    ...
    public boolean isXHR() {
        return headers.contains("X-Requested-With") && !headers.get("X-Requested-With").isEmpty() && headers.get("X-Requested-With").get(0).equals("XMLHttpRequest");
    }
